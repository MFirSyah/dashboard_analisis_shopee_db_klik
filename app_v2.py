# ===================================================================================
#  DASHBOARD ANALISIS PENJUALAN & KOMPETITOR - VERSI 3 (GABUNGAN)
#  Dibuat oleh: Firman & Asisten AI Gemini
#  Prinsip:
#    - Mesin Pengambilan Data: Otomatis & Canggih dari app_v2.py
#    - Otak Analisis & Perhitungan: Akurat & Teruji dari app.py
# ===================================================================================
...
# --- KONFIGURASI HALAMAN ---
st.set_page_config(layout="wide", page_title="Dashboard Analisis Penjualan v3")

# --- KONFIGURASI ID & NAMA KOLOM (Standarisasi) ---
# Ini penting untuk konsistensi di seluruh aplikasi
PARENT_FOLDER_ID = "1z0Ex2Mjw0pCWt6BwdV1OhGLB8TJ9EPWq" # Ganti dengan ID Folder Induk Anda
SPREADSHEET_ID = "1iX-LpYJrHRqD5-c2-D27kVY7PArYLaSCCd-nvd2y6Yg" # Ganti dengan ID Spreadsheet "Otak" Anda

DB_SHEET_NAME = "database_brand"
KAMUS_SHEET_NAME = "kamus_brand"
KATEGORI_SHEET_NAME = "DATABASE"

# Nama Kolom yang digunakan di seluruh aplikasi
...
# --- FUNGSI-FUNGSI INTI (Diambil dari app_v2.py yang lebih superior) ---
...
def get_all_competitor_data(_drive_service, parent_folder_id):
    """
    Fungsi "Anti Peluru": Membaca file CSV & Google Sheets dari semua subfolder, 
    melewati file yang error, dan memberikan laporan diagnosis.
    """
    all_data = []
    problematic_files = []
    try:
            ...
            for file_item in files_in_folder:
                try:
                    ...
                    if NAMA_PRODUK_COL not in df.columns:
                        problematic_files.append(f"{folder['name']}/{file_name} (Kolom '{NAMA_PRODUK_COL}' tidak ditemukan)"); continue
                    ...
                    df[STATUS_COL] = 'Tersedia' if 'ready' in file_name.lower() else 'Habis' if 'habis' in file_name.lower() else 'N/A'
                        
                    all_data.append(df)
                except Exception as file_error:
                    problematic_files.append(f"{folder['name']}/{file_name} (Error: {file_error})"); continue
        ...
        # Pembersihan dan konversi tipe data yang lebih aman
        for col in [HARGA_COL, TERJUAL_COL]:
            if col not in final_df.columns: final_df[col] = 0
            else:
                final_df[col] = final_df[col].astype(str).str.replace(r'[^\d.]', '', regex=True) # Memperbolehkan titik untuk desimal
                final_df[col] = pd.to_numeric(final_df[col], errors='coerce').fillna(0)
        
        final_df[TERJUAL_COL] = final_df[TERJUAL_COL].astype(int)
        final_df[HARGA_COL] = final_df[HARGA_COL].astype(float).astype(int)
...
def label_brands(df, brand_db, kamus_brand, fuzzy_threshold=88):
    """Fungsi untuk melabeli brand berdasarkan nama produk, menggunakan kamus dan fuzzy matching."""
    ...
    for _, row in df.iterrows():
        ...
        # 1. Cek Kamus Alias (prioritas tertinggi)
        ...
        # 2. Cek Database Brand (pencocokan eksak)
        ...
        # 3. Fuzzy Matching (jika tidak ada yang cocok)
...
# --- FUNGSI BANTU ANALISIS (Diambil dari app.py & app_v2.py) ---
def format_wow_growth(pct_change):
    """Format pertumbuhan WoW dengan ikon panah."""
    if pd.isna(pct_change) or pct_change == float('inf'): return "N/A"
    elif pct_change > 0.001: return f"▲ {pct_change:.1%}"
    elif pct_change < -0.001: return f"▼ {pct_change:.1%}"
    else: return f"▬ {pct_change:.1%}"

def format_harga_aman(x):
    """Format angka menjadi format Rupiah yang aman dari error."""
    if pd.isnull(x): return "N/A"
    try: return f"Rp {float(x):,.0f}"
    except (ValueError, TypeError): return str(x)

def colorize_growth(val):
    """Memberi warna pada teks pertumbuhan."""
    if isinstance(val, str):
        if '▲' in val: return 'color: #28a745' # Hijau
        elif '▼' in val: return 'color: #dc3545' # Merah
...
st.title("📊 Dashboard Analisis Penjualan & Kompetitor v3")

# Memuat data utama sekali di awal
master_df, brand_db, kamus_brand, db_kategori, problematic_files = load_master_data()

# Menampilkan laporan file bermasalah jika ada
...
if master_df is None or master_df.empty:
    st.error("Gagal memuat data utama atau tidak ada data valid yang ditemukan. Cek folder Google Drive atau file CSV.")
    st.stop()
...
# Filter data yang brand-nya sudah teridentifikasi
...
try:
    ...
    if df_with_dates.empty:
        st.error("Tidak ada data dengan tanggal yang valid. Pastikan nama file CSV/Sheet mengandung tanggal format YYYY-MM-DD.")
        st.stop()
...
if df_filtered.empty:
    st.warning("Tidak ada data pada rentang tanggal yang dipilih."); st.stop()

df_filtered[MINGGU_COL] = df_filtered[TANGGAL_COL].dt.to_period('W-SUN').apply(lambda p: p.start_time).dt.date
...
if page == "Analisis Penjualan":
    ...
    with tab1: # Analisis Toko Saya
        ...
        if not db_kategori.empty and 'KATEGORI' in db_kategori.columns and 'NAMA' in db_kategori.columns:
            ...
            def fuzzy_merge_categories(_rekap_df, _database_df):
                _rekap_df[KATEGORI_COL] = 'Lainnya'
                ...
                for index, row in _rekap_df.iterrows():
                    if pd.notna(row[NAMA_PRODUK_COL]):
                        ...
                        if score >= 95: _rekap_df.loc[index, KATEGORI_COL] = db_map[match]
                return _rekap_df
            ...
            if not category_sales.empty:
                ...
                fig_cat = px.bar(cat_sales_sorted, x=KATEGORI_COL, y=TERJUAL_COL, title=f'Top {top_n_cat} Kategori', text_auto=True)
    ...
    with tab2: # Perbandingan Harga (Logika dari app.py)
        ...
        if not main_store_df.empty:
            ...
            main_store_latest_display = main_store_latest[[TANGGAL_COL, NAMA_PRODUK_COL, HARGA_COL, STATUS_COL]].copy()
            main_store_latest_display[HARGA_COL] = main_store_latest_display[HARGA_COL].apply(format_harga_aman)
            st.dataframe(main_store_latest_display, use_container_width=True, hide_index=True)
            
            st.subheader("2. Pilih Produk untuk Dibandingkan")
            search_query = st.text_input("Cari produk berdasarkan nama:", key="search_product")
            ...
            if not product_list:
                ...
                if selected_product:
                    ...
                    col1.metric(f"Harga di {main_store}", format_harga_aman(product_info[HARGA_COL]))
                    col2.metric(f"Status", product_info[STATUS_COL])
                    
                    st.markdown("---")
                    st.markdown(f"**Perbandingan di Toko Kompetitor (Data per {latest_date.strftime('%Y-%m-%d')}):**")
                    ...
                    if not competitor_latest.empty:
                        ...
                        if not valid_matches:
                            ...
                            for product_match, score in valid_matches:
                                match_info = competitor_latest[competitor_latest[NAMA_PRODUK_COL] == product_match].iloc[0]
                                
                                # Perhitungan selisih harga yang aman
                                price_diff = int(match_info[HARGA_COL]) - int(product_info[HARGA_COL])
    ...
    with tab3: # Analisis Brand Kompetitor (Logika dari app.py)
        st.header("Analisis Brand di Toko Kompetitor")
        if competitor_df.empty:
            st.warning("Tidak ada data kompetitor pada rentang tanggal ini.")
        else:
            competitor_list = sorted(competitor_df[TOKO_COL].unique())
            for competitor_store in competitor_list:
                with st.expander(f"Analisis untuk Kompetitor: **{competitor_store}**"):
                    ...
                    brand_analysis = single_competitor_df.groupby(BRAND_COL).agg(
                        ...
                        Total_Unit_Terjual=(TERJUAL_COL, 'sum')
                    ...
                    with col1:
                        ...
                        brand_analysis_display['Total_Omzet'] = brand_analysis_display['Total_Omzet'].apply(format_harga_aman)
                        st.dataframe(brand_analysis_display[[BRAND_COL, 'Total_Unit_Terjual', 'Total_Omzet']], use_container_width=True, hide_index=True)
                    with col2:
    ...
    with tab4: # Status Stok Produk
        st.header("Tren Status Stok Mingguan per Toko")
        stock_trends = df_filtered.groupby([MINGGU_COL, TOKO_COL, STATUS_COL]).size().unstack(fill_value=0).reset_index()
        if 'Tersedia' not in stock_trends.columns: stock_trends['Tersedia'] = 0
        if 'Habis' not in stock_trends.columns: stock_trends['Habis'] = 0
        stock_trends_melted = stock_trends.melt(id_vars=[MINGGU_COL, TOKO_COL], value_vars=['Tersedia', 'Habis'], var_name='Tipe Stok', value_name='Jumlah Produk')
        fig_stock_trends = px.line(stock_trends_melted, x=MINGGU_COL, y='Jumlah Produk', color=TOKO_COL, line_dash='Tipe Stok', markers=True, title='Jumlah Produk Tersedia vs. Habis per Minggu')
        st.plotly_chart(fig_stock_trends, use_container_width=True)
        st.dataframe(stock_trends.set_index(MINGGU_COL), use_container_width=True)

    with tab5: # Kinerja Penjualan
        st.header("Analisis Kinerja Penjualan (Semua Toko)")
        
        st.subheader("1. Grafik Omzet Mingguan")
        weekly_omzet = df_filtered.groupby([MINGGU_COL, TOKO_COL])[OMZET_COL].sum().reset_index()
        fig_weekly_omzet = px.line(weekly_omzet, x=MINGGU_COL, y=OMZET_COL, color=TOKO_COL, markers=True, title='Perbandingan Omzet Mingguan Antar Toko')
        ...
        for store in sorted(df_filtered[TOKO_COL].unique()):
            ...
            weekly_summary = store_df.groupby(MINGGU_COL).agg(
                Total_Omzet=(OMZET_COL, 'sum'),
        ...
        if summary_list:
            ...
            # Perbaikan rumus: rata-rata harian dibagi 7
            ...
            final_summary_display['Total_Omzet'] = final_summary_display['Total_Omzet'].apply(format_harga_aman)
            final_summary_display['Rata_Rata_Harga'] = final_summary_display['Rata_Rata_Harga'].apply(format_harga_aman)
            
            display_cols = {
                MINGGU_COL: 'Mulai Minggu', TOKO_COL: 'Toko', 'Total_Omzet': 'Total Omzet',
                'Pertumbuhan Omzet (WoW)': 'Pertumbuhan Omzet (WoW)', 'Total_Terjual': 'Total Unit Terjual',
                'Rata_Rata_Terjual_Harian': 'Rata-Rata Terjual Harian', 'Rata_Rata_Harga': 'Rata-Rata Harga'
            }
            st.dataframe(final_summary_display[list(display_cols.keys())].rename(columns=display_cols)
                         .style.applymap(colorize_growth, subset=['Pertumbuhan Omzet (WoW)']),
                         use_container_width=True, hide_index=True)

    with tab6: # Analisis Mingguan
        st.header("Analisis Produk Baru Mingguan")
        st.subheader("Perbandingan Produk Baru Antar Minggu")
        weeks = sorted(df_filtered[MINGGU_COL].unique())
        if len(weeks) < 2:
            ...
            if week_before >= week_after:
                ...
                for store in sorted(df_filtered[TOKO_COL].unique()):
                    with st.expander(f"Lihat Produk Baru di Toko: **{store}**"):
                        products_before = set(df_filtered[(df_filtered[TOKO_COL] == store) & (df_filtered[MINGGU_COL] == week_before)][NAMA_PRODUK_COL])
                        products_after = set(df_filtered[(df_filtered[TOKO_COL] == store) & (df_filtered[MINGGU_COL] == week_after)][NAMA_PRODUK_COL])
                        ...
                        if not new_products:
                            ...
                            new_products_df = df_filtered[df_filtered[NAMA_PRODUK_COL].isin(new_products) & (df_filtered[TOKO_COL] == store) & (df_filtered[MINGGU_COL] == week_after)]
    ...
    try:
        _, gsheets_service = get_google_apis()
    except Exception as e:
        st.error(f"Gagal terhubung ke Google Sheets untuk melakukan update: {e}")
        st.stop()
    ...
    if unknown_df.empty:
        ...
        with st.form(key="review_form"):
            ...
            # Ambil daftar brand terbaru dari session state
            current_brands = sorted(st.session_state.brand_db)
            selected_brand = col1.selectbox("Pilih dari brand yang sudah ada:", options=[""] + current_brands)
            ...
            if submitted:
                ...
                final_brand = selected_brand or new_brand_input.strip().upper()

                if not final_brand:
                    st.error("Anda harus memilih brand yang sudah ada atau memasukkan brand baru.")
                elif alias_input: # Jika alias diisi, ini adalah prioritas
                ...
                elif new_brand_input: # Jika hanya brand baru diisi
                    if new_brand_input.strip().upper() not in current_brands:
                        if update_google_sheet(gsheets_service, SPREADSHEET_ID, DB_SHEET_NAME, [final_brand]):
                            st.success(f"Brand baru ditambahkan: '{final_brand}'.")
                            correction_made = True
                    else:
                        st.warning(f"Brand '{final_brand}' sudah ada di database. Jika ini adalah alias, gunakan kolom Alias.")
                else:
                     st.error("Untuk mengajari sistem, Anda harus mengisi 'Alias' atau 'Brand Baru'.")

                if correction_made:
                    st.success("Sistem sedang mempelajari data baru... Halaman akan dimuat ulang.")
                    st.cache_data.clear()
